...
OUTPUT_ROOT = "/media/kpeng/Elements/DATA/data_collection/four_robot"
...
SAME_FLOOR_Y_TOL = 0.25
START_OTHER_Y_TRIES = 1000

# Start yaw aligned-ish
START_YAW_ALIGN_DEG = 8.0
START_YAW_JITTER_DEG = 10.0

# Minimum spacing among starting robots (XZ plane)
START_MIN_PAIR_DIST_M = 2
START_MAX_PAIR_DIST_M = 10
...
MEET_EVERY_STEPS = 220
MEET_DURATION_STEPS = 90
MEET_TARGET_RADIUS_M = 1.8

TETHER_ENABLED = True
TETHER_MAX_DIST = 12.0
TETHER_PULL_PROB = 0.70
...
# Critical fix: keep robot body slightly above navmesh so it does not clip / sink / get stuck.
ROBOT_BODY_Y_OFFSET = 0.18
...
def run_one_exploration(scene_info: dict, explore_idx: int):
    ...
    for rid in range(1, NUM_ROBOTS):
        ...
        if start_region is not None:
            for _ in range(START_OTHER_Y_TRIES):
                cand = sample_navigable_point_in_region_same_floor(
                    pf, start_region, ref_y=base_y, max_tries=40, y_tol=SAME_FLOOR_Y_TOL
                )
                if cand is None:
                    continue

                ok_dist = True
                for prev in start_points:
                    d = xz_dist(cand, prev)
                    if d < START_MIN_PAIR_DIST_M or d > START_MAX_PAIR_DIST_M:
                        ok_dist = False
                        break
                if ok_dist:
                    pi = cand
        ...
        if pi is None:
            for _ in range(START_OTHER_Y_TRIES):
                cand = sample_navigable_point_same_floor(
                    pf, ref_y=base_y, max_tries=40, y_tol=SAME_FLOOR_Y_TOL
                )
                if cand is None:
                    continue
                ok_dist = True
    ...
    with open(os.path.join(out_dir, "meta.txt"), "w") as f:
        ...
        f.write(f"robot_body_y_offset: {ROBOT_BODY_Y_OFFSET}\n")
    ...
    def nav_constrained_step(x, y, z, yaw, v, w):
        ...
        for k in range(NAV_STEP_TRIES):
            ...
            if pf.is_navigable(snapped) and abs(float(snapped.y) - float(y)) <= SAME_FLOOR_Y_TOL:
    ...
    try:
        for t in range(NUM_STEPS):
            ...
            # Write body poses with safe body height above navmesh
            for i in range(NUM_ROBOTS):
                x, y, z = positions[i]
                yaw = yaws[i]

                snapped_pose = pf.snap_point(mn.Vector3(float(x), float(y), float(z)))
                nav_x = float(snapped_pose.x)
                nav_y = float(snapped_pose.y)
                nav_z = float(snapped_pose.z)

                positions[i] = np.array([nav_x, nav_y, nav_z], dtype=np.float32)

                locobots[i].translation = mn.Vector3(nav_x, nav_y + ROBOT_BODY_Y_OFFSET, nav_z)
