...
OUTPUT_ROOT        = "/media/kpeng/Elements/DATA/data_collection/four_robot_igibson"
...
def build_sim():
    ...
    r4 = Locobot(action_type="continuous",action_normalize=False)
    sim.import_object(r1); sim.import_object(r2); sim.import_object(r3); sim.import_object(r4)
    try:
        d = r1.action_space.shape[0]
        print(f"  Locobot action_dim={d} (expected {LOCOBOT_DIM})")
    except Exception: pass
    return sim, scene, r1, r2, r3, r4
...
BEV_C1=(255,80,80); BEV_C2=(80,80,255); BEV_C3=(80,200,80); BEV_C4=(200,150,50)
...
class BEV:
    ...
    def frame(self,t,x1,z1,y1,x2,z2,y2,x3,z3,y3,x4,z4,y4,obj_xz=None):
        C=BEV_SIZE; cv=np.full((C,C,3),40,np.uint8)
        for rid,xr,zr in ((1,x1,z1),(2,x2,z2),(3,x3,z3),(4,x4,z4)):
        ...
        for rid,col in ((1,BEV_C1),(2,BEV_C2),(3,BEV_C3),(4,BEV_C4)):
            pts=self.trails[rid]; n=len(pts)
            for i in range(1,n):
                a=i/n; c=tuple(int(v*a) for v in col)
                _dl(cv,self._p(*pts[i-1]),self._p(*pts[i]),c,2)
        for xr,zr,yr,col in ((x1,z1,y1,BEV_C1),(x2,z2,y2,BEV_C2),(x3,z3,y3,BEV_C3),(x4,z4,y4,BEV_C4)):
...
def run_exploration(scene_name: str, explore_idx: int):
    ...
    sim, scene, r1, r2, r3, r4 = build_sim()
    ...
    rng4  = random.Random((seed+404)&0xFFFFFFFF)

    # ── same-floor start ──────────────────────────────────────
    p1, p2, p3, p4 = choose_starts(scene, rng_g)
    ...
    y4 = wrap(y1 + math.radians(rng4.uniform(-START_YAW_ALIGN_DEG,START_YAW_ALIGN_DEG)))
    ...
    # waypoints
    visited1: Set = set(); visited2: Set = set(); visited3: Set = set(); visited4: Set = set()
    ...
    wp4 = wp_global(scene,rng4,np.array([x4,y4f,z4],np.float32),visited4)
    ...
    wpt4 = rng4.randint(WP_MIN_STEPS,WP_MAX_STEPS)

    # wiggle counters
    wig1 = wig2 = wig3 = wig4 = 0

    # wall-turn counters
    wall_turn1=0; wall_turn2=0; wall_turn3=0; wall_turn4=0
    wall_dir1=1.0; wall_dir2=1.0; wall_dir3=1.0; wall_dir4=1.0
    ...
    tgt4: Optional[TObj]=None; tgt_t4=0
    ...
    push_state4=PUSH_IDLE; push_tobj4: Optional[TObj]=None; push_timer4=0
    ...
    vw = ({
        "rgb1":_w("locobot1_cam.mp4"),  "rgb2":_w("locobot2_cam.mp4"),  "rgb3":_w("locobot3_cam.mp4"),  "rgb4":_w("locobot4_cam.mp4"),
        "dep1":_w("locobot1_depth.mp4"),"dep2":_w("locobot2_depth.mp4"),"dep3":_w("locobot3_depth.mp4"),"dep4":_w("locobot4_depth.mp4"),
        "sem1":_w("locobot1_sem.mp4"),  "sem2":_w("locobot2_sem.mp4"),  "sem3":_w("locobot3_sem.mp4"),  "sem4":_w("locobot4_sem.mp4"),
    ...
    tw.writerow(["t","robot_id",
                 ...
                 "sees_r2","dist_r2","sees_r3","dist_r3","sees_r4","dist_r4",
    ...
    try:
        for t in range(NUM_STEPS):
            ...
            d14 = math.sqrt((x4-x1)**2+(z4-z1)**2)
            d23 = math.sqrt((x3-x2)**2+(z3-z2)**2)
            d24 = math.sqrt((x4-x2)**2+(z4-z2)**2)
            d34 = math.sqrt((x4-x3)**2+(z4-z3)**2)
            phase_timer -= 1
            if phase_timer <= 0:
                ...
                print(f"  [t={t}] → {phase_str} phase d12={d12:.1f} d13={d13:.1f} d14={d14:.1f} d23={d23:.1f} d24={d24:.1f} d34={d34:.1f}")
            ...
            if phase_near:
                ...
                if obs_a <= 0 and near_elapsed == PHASE_NEAR_STEPS // 2:
                    ...
                    obs_who = 1 + (t // PHASE_NEAR_STEPS) % 4  # cycles 1→2→3→4→1
            if obs_a > 0:
                obs_a -= 1
                if obs_a <= 0: obs_who = 0

            # ── waypoint update ─────────────────────────────────────
            wpt1 -= 1; wpt2 -= 1; wpt3 -= 1; wpt4 -= 1

            if phase_near:
                # ── NEAR phase: all three navigate toward shared centroid ──
                cx = (x1+x2+x3+x4)/4.; cy = (y1f+y2f+y3f+y4f)/4.; cz = (z1+z2+z3+z4)/4.
                ...
                if obs_who == 3:
                    ...
                    wpt3 = rng3.randint(WP_MIN_STEPS,WP_MAX_STEPS)
                if d13 > NEAR_MAX_DIST and d23 > NEAR_MAX_DIST and d34 > NEAR_MAX_DIST and rng_g.random()<0.9:
                    wp3 = wp_near(scene,rng3,mid,2.0); wpt3=rng3.randint(15,40)


                if obs_who == 4:
                    wp4 = np.array([x1,y1f,z1],np.float32); wpt4=5
                elif wpt4 <= 0:
                    wp4 = wp_near(scene,rng4,mid,NEAR_WP_RADIUS)
                    wpt4 = rng4.randint(WP_MIN_STEPS,WP_MAX_STEPS)
                if d14 > NEAR_MAX_DIST and d24 > NEAR_MAX_DIST and d34 > NEAR_MAX_DIST and rng_g.random()<0.9:
                    wp4 = wp_near(scene,rng4,mid,2.0); wpt4=rng4.randint(15,40)

            else:
                # ── FAR phase: four robots explore independently ───────
                if wpt1 <= 0:
                    if tgt1 is not None:
                        tp=tgt1.pos()
                ...
                if wpt4 <= 0:
                    if tgt4 is not None:
                        tp=tgt4.pos()
                        if tp is not None: wp4=tp.copy()
                        else: wp4=wp_global(scene,rng4,np.array([x4,y4f,z4],np.float32),visited4,FAR_WP_DIST_MIN,FAR_WP_DIST_MAX)
                    else: wp4=wp_global(scene,rng4,np.array([x4,y4f,z4],np.float32),visited4,FAR_WP_DIST_MIN,FAR_WP_DIST_MAX)
                    wpt4=rng4.randint(WP_MIN_STEPS,WP_MAX_STEPS)
            ...
            # ── Robot 4 push FSM ─────────────────────────────────
            if push_state4 == PUSH_IDLE:
                nearby_factor4 = 1.0 + 3.0 * float(min(d14,d24,d34) < 5.0)
                if pushable and rng4.random() < PUSH_PROB * nearby_factor4:
                    push_tobj4  = rng4.choice(pushable)
                    push_state4 = PUSH_APPROACH
                    push_timer4 = PUSH_APPROACH_MAX

            elif push_state4 == PUSH_APPROACH:
                tp = push_tobj4.pos() if push_tobj4 else None
                if tp is not None:
                    wp4 = tp.copy()
                    push_timer4 -= 1
                    if dxz(x4,z4,float(tp[0]),float(tp[2])) < PUSH_DIST_TRIGGER:
                        push_state4 = PUSH_PUSHING
                        push_timer4 = PUSH_FORCE_STEPS
                        if not push_observed:
                            push_observe_timer = PUSH_OBSERVE_STEPS
                    elif push_timer4 <= 0:
                        push_state4 = PUSH_IDLE; push_tobj4 = None
                else:
                    push_state4 = PUSH_IDLE; push_tobj4 = None

            elif push_state4 == PUSH_PUSHING:
                if push_tobj4 is not None:
                    apply_push_force(push_tobj4, np.array([x4,y4f,z4],np.float32))
                    wp4 = push_tobj4.pos().copy() if push_tobj4.pos() is not None else wp4
                push_timer4 -= 1
                if push_timer4 <= 0:
                    push_state4 = PUSH_IDLE; push_tobj4 = None
            ...
            if (push_state4==PUSH_IDLE and tgt4 is None and pushable and rng4.random()<MOVE_OBJ_PROB):
                tgt4=rng4.choice(all_obj); tgt_t4=MOVE_OBJ_DURATION
            if tgt_t4>0:
                tgt_t4-=1
                if tgt_t4<=0: tgt4=None
                elif tgt4 is not None and push_state4==PUSH_IDLE:
                    tp=tgt4.pos()
                    if tp is not None:
                        wp4=tp.copy()
                        if dxz(x4,z4,float(tp[0]),float(tp[2]))<INTERACT_DIST_M: tgt4=None; tgt_t4=0
            ...
            if wig4<=0 and rng4.random()<WIGGLE_PROB: wig4=rng4.randint(WIGGLE_MIN_STEPS,WIGGLE_MAX_STEPS)
            ...
            if WALL_RAY_ENABLED:
                ...
                if wall_turn4<=0 and wall_ray_detect(r4,yaw4):
                    wall_dir4=1.0 if rng4.random()<0.5 else -1.0; wall_turn4=WALL_TURN_STEPS
                    wp4=wp_near(scene,rng4,np.array([x4,y4f,z4],np.float32),4.0); wpt4=rng4.randint(WP_MIN_STEPS,WP_MAX_STEPS)

            # ── compute base commands (all robots) ────────────────
            v1,w1 = cmd(x1,z1,yaw1,wp1,rng1)
            v2,w2 = cmd(x2,z2,yaw2,wp2,rng2)
            v3,w3 = cmd(x3,z3,yaw3,wp3,rng3)
            v4,w4 = cmd(x4,z4,yaw4,wp4,rng4)
            ...
            if wall_turn4>0: v4=FORWARD_SPEED*0.25; w4=wall_dir4*ROT_SPEED*0.7; wall_turn4-=1
            ...
            if push_observe_timer > 0:
                ...
                if push_state1 == PUSH_PUSHING:
                    ...
                    wp4 = np.array([x2,y2f,z2],np.float32); wpt4=5
                    obs_who = 1
                elif push_state3 == PUSH_PUSHING:
                    wp1 = np.array([x3,y3f,z3],np.float32); wpt1=5
                    wp2 = np.array([x3,y3f,z3],np.float32); wpt2=5
                    wp4 = np.array([x3,y3f,z3],np.float32); wpt4=5
                    obs_who = 1
                elif push_state4 == PUSH_PUSHING:
                    wp1 = np.array([x4,y4f,z4],np.float32); wpt1=5
                    wp2 = np.array([x4,y4f,z4],np.float32); wpt2=5
                    wp3 = np.array([x4,y4f,z4],np.float32); wpt3=5
                    obs_who = 1
                if push_observe_timer <= 0:
                    push_observed = True
            ...
            if wig4>0: w4=rng4.uniform(-WIGGLE_TURN_MAG,WIGGLE_TURN_MAG)/DT; v4=FORWARD_SPEED*WIGGLE_FWD_SCALE; wig4-=1

            v1=max(0.,min(FORWARD_SPEED,v1)); v2=max(0.,min(FORWARD_SPEED,v2)); v3=max(0.,min(FORWARD_SPEED,v3)); v4=max(0.,min(FORWARD_SPEED,v4))
            w1=max(-ROT_SPEED,min(ROT_SPEED,w1)); w2=max(-ROT_SPEED,min(ROT_SPEED,w2)); w3=max(-ROT_SPEED,min(ROT_SPEED,w3)); w4=max(-ROT_SPEED,min(ROT_SPEED,w4))
            ...
            x4,y4f,z4,yaw4 = nav_step(scene,x4,y4f,z4,yaw4,v4,w4)
            ...
            if (x4-prev_x4)**2+(z4-prev_z4)**2 < STUCK_EPS_SQ: stuck4+=1
            else: stuck4=0
            prev_x1,prev_z1=x1,z1; prev_x2,prev_z2=x2,z2; prev_x3,prev_z3=x3,z3; prev_x4,prev_z4=x4,z4
            ...
            if stuck4>STUCK_SOFT_LIMIT:
                wp4=wp_global(scene,rng4,np.array([x4,y4f,z4],np.float32),visited4)
                wpt4=rng4.randint(WP_MIN_STEPS,WP_MAX_STEPS); wig4=rng4.randint(WIGGLE_MIN_STEPS,WIGGLE_MAX_STEPS)
                stuck4=0; tgt4=None; tgt_t4=0
            ...
            if HARD_STUCK_TELEPORT and stuck3>STUCK_HARD_LIMIT:
                ...
                stuck3=0; tgt3=None; tgt_t3=0
            if HARD_STUCK_TELEPORT and stuck4>STUCK_HARD_LIMIT:
                np4=floor_pt(scene,rng4); x4,y4f,z4=float(np4[0]),float(np4[1]),float(np4[2])
                yaw4=rng4.uniform(-math.pi,math.pi); init_robot(r4,np4,yaw4,sim,20)
                wp4=wp_global(scene,rng4,np4,visited4); wpt4=rng4.randint(WP_MIN_STEPS,WP_MAX_STEPS)
                stuck4=0; tgt4=None; tgt_t4=0
            ...
            _safe_apply(r4,nav_action(v4,w4,robot_idx=1))
            sim.step()
            ...
            vis14,d14v = check_vis(r1,r4,yaw1)
            vis24,d24v = check_vis(r2,r4,yaw2)
            vis31,d31v = check_vis(r3,r1,yaw3)
            vis32,d32v = check_vis(r3,r2,yaw3)
            vis34,d34v = check_vis(r3,r4,yaw3)
            vis41,d41v = check_vis(r4,r1,yaw4)
            vis42,d42v = check_vis(r4,r2,yaw4)
            vis43,d43v = check_vis(r4,r3,yaw4)
            # observed_by: comma-sep list of robot ids that see this robot
            def _ob(sees_list): return ",".join(str(r) for r,v in sees_list if v)
            ob1=_ob([(2,vis21),(3,vis31),(4,vis41)]); ob2=_ob([(1,vis12),(3,vis32),(4,vis42)])
            ob3=_ob([(1,vis13),(2,vis23),(4,vis43)]); ob4=_ob([(1,vis14),(2,vis24),(3,vis34)])
            oi1=_ob([(2,vis12),(3,vis13),(4,vis14)]); oi2=_ob([(1,vis21),(3,vis23),(4,vis24)])
            oi3=_ob([(1,vis31),(2,vis32),(4,vis34)]); oi4=_ob([(1,vis41),(2,vis42),(3,vis43)])
            ...
            seg4=np.zeros((IMAGE_HEIGHT,IMAGE_WIDTH),np.int32)
            ...
            if SAVE_VIDEO and t%max(1,SAVE_EVERY)==0:
                ...
                seg4, wp4_new = _render_and_write(r4,["rgb4","dep4","sem4"],rng4,x4,y4f,z4)
                if wp4_new is not None: wp4=wp4_new

            if SAVE_VIDEO and t%max(1,BEV_EVERY)==0:
                oxz=[(float(o.pos()[0]),float(o.pos()[2])) for o in all_obj if o.pos() is not None]
                vw["bev"].append_data(bev.frame(t,x1,z1,yaw1,x2,z2,yaw2,x3,z3,yaw3,x4,z4,yaw4,oxz))
            ...
            v4s=visible_ids(seg4,VISIBLE_MIN_PIX,VIS_DOWNSAMPLE)
            nb1=nearby(x1,z1,yaw1,all_obj,v1s)
            nb2=nearby(x2,z2,yaw2,all_obj,v2s)
            nb3=nearby(x3,z3,yaw3,all_obj,v3s)
            nb4=nearby(x4,z4,yaw4,all_obj,v4s)
            for rid,nb in ((1,nb1),(2,nb2),(3,nb3),(4,nb4)):
                for it in nb:
            ...
            push_name4 = push_tobj4.name if push_state4 == PUSH_PUSHING and push_tobj4 else ""
            ...
            _pw(1, push_name1, push_tobj1, x1, z1, vis21 or vis31 or vis41, min(d21v,d31v,d41v))
            _pw(2, push_name2, push_tobj2, x2, z2, vis12 or vis32 or vis42, min(d12v,d32v,d42v))
            _pw(3, push_name3, push_tobj3, x3, z3, vis13 or vis23 or vis43, min(d13v,d23v,d43v))
            _pw(4, push_name4, push_tobj4, x4, z4, vis14 or vis24 or vis34, min(d14v,d24v,d34v))

            # ── trajectory CSV ─────────────────────────────────────
            an1=albl(v1,w1); an2=albl(v2,w2); an3=albl(v3,w3); an4=albl(v4,w4)
            ...
            tw.writerow([t,4,x4,y4f,z4,yaw4,v4,w4,an4,int(vis41),d41v,int(vis42),d42v,ob4,oi4,push_name4])
            ...
            jf.write(json.dumps({
                ...
                "robots":[
                    _rdata(1,x1,y1f,z1,yaw1,v1,w1,an1,push_state1,push_name1,ob1,oi1,nb1,
                           {"r2":vis12,"r3":vis13,"r4":vis14},{"r2":d12v,"r3":d13v,"r4":d14v}),
                    _rdata(2,x2,y2f,z2,yaw2,v2,w2,an2,push_state2,push_name2,ob2,oi2,nb2,
                           {"r1":vis21,"r3":vis23,"r4":vis24},{"r1":d21v,"r3":d23v,"r4":d24v}),
                    _rdata(3,x3,y3f,z3,yaw3,v3,w3,an3,push_state3,push_name3,ob3,oi3,nb3,
                           {"r1":vis31,"r2":vis32,"r4":vis34},{"r1":d31v,"r2":d32v,"r4":d34v}),
                    _rdata(4,x4,y4f,z4,yaw4,v4,w4,an4,push_state4,push_name4,ob4,oi4,nb4,
                           {"r1":vis41,"r2":vis42,"r3":vis43},{"r1":d41v,"r2":d42v,"r3":d43v}),
                ]})+"\n")

            if (t+1)%200==0:
                print(f"  [{t+1}/{NUM_STEPS}] phase={'NEAR' if phase_near else 'FAR'} "
                      f"d12={d12:.1f} d13={d13:.1f} d23={d23:.1f} "
                      f"stuck={stuck1}/{stuck2}/{stuck3}/{stuck4} "
                      f"vis={len(visited1)}/{len(visited2)}/{len(visited3)}/{len(visited4)} "
                      f"push='{push_name1}'/'{push_name2}'/'{push_name3}'/'{push_name4}'")
